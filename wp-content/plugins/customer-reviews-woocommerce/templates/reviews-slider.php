...
<div class="cr-reviews-slider" id="<?php echo $id; ?>" data-slick='<?php echo wp_json_encode( $slider_settings ); ?>' style="<?php echo $section_style; ?>">
	...
	?>
		<div class="ivole-review-card">
            ...
            if( $max_chars && strlen( $clear_content ) > $max_chars ) {
              $less_content = substr( $clear_content, 0, $max_chars );
              $more_content = substr( $clear_content, $max_chars );
              $read_more = '<span class="cr-slider-read-more">...<br><a href="#">' . esc_html( 'Show More', 'customer-reviews-woocommerce' ) . '</a></span>';
              $more_content = '<span class="cr-slider-details" style="display:none;">' . $more_content . '<span class="cr-slider-read-less"><br><a href="#">' . esc_html( 'Show Less', 'customer-reviews-woocommerce' ) . '</a></span></span>';
              $comment_content = $less_content . $read_more . $more_content;
              echo wpautop( wp_kses_post( $comment_content ) );
            } else {
